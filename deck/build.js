...
{ const s = pres.addSlide(); light(s); n++;
  ...
  card(s, M, 1.9, 6.0, 4.85);
  s.addText("Train on RadioML. Test on I/Q from our own transmitter model: same five modulations, same SNR grid, same 1,024-sample frames at 8 samples per symbol.",
    { x: M + 0.3, y: 2.05, w: 5.4, h: 0.95, fontFace: BODY, fontSize: 14, color: INK, isTextBox: true, margin: 0, valign: "top" });
  stat(s, M + 0.3, 3.0, 2.5, f3(none.in_dom), "in-domain accuracy", TEAL, 40);
  stat(s, M + 3.0, 3.0, 2.5, f3(none.cross), "on the other transmitter", RED, 40);
  s.addText(`16QAM alone: ${f3(none.qam16)} — it is read as 64QAM`, { x: M + 0.3, y: 4.75, w: 5.4, h: 0.4, fontFace: BODY, fontSize: 13, bold: true, color: INK, isTextBox: true, margin: 0 });
  const hc = t => ({ text: t, options: { bold: true, fill: { color: PALE }, color: INK, fontSize: 9 } });
  s.addTable([[hc(""), hc("RadioML 2018 — trained on"), hc("our generator — tested on")],
    [{ text: "pulse shaping", options: { bold: true } }, "root-raised cosine, roll-off drawn per frame from U(0.1, 0.4)", "root-raised cosine, roll-off fixed at 0.35"],
    [{ text: "channel", options: { bold: true } }, "carrier, clock and timing offsets; Rayleigh multipath", "none"],
    [{ text: "noise", options: { bold: true } }, "AWGN, −20 to +30 dB", "AWGN, same grid"]],
    { x: M + 0.3, y: 5.25, w: 5.4, colW: [1.05, 2.3, 2.05], fontFace: BODY, fontSize: 9, color: INK, border: { type: "solid", pt: 0.5, color: "D5DDE5" }, rowH: 0.3, autoPage: false });
